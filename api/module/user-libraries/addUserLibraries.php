...
if ($bydirect) { 
     ...
     if (isset($_REQUEST['source']) && isset($_REQUEST['details'])) {
          ...
          //if ($query_result->num_rows == 0) {
               $query = "INSERT INTO user_libraries(user_id,source_id,details,status,created) VALUES ('".$_SESSION['user']['user_id']."','$source','$details','$status','$created')";
               $query_result = $con->query($query);
               $result['success'] = 1;
               $result['data'] = 'success';
               $result['error'] = 0;
