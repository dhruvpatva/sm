...
SoundMobApp.controller('PostCommentsController', function($scope, $http, $timeout, $location, $state, $stateParams, Upload) {
    ...
    function getRecords(pageNumber) {
        ...
        if (!$.isEmptyObject($scope.libraryTemp)) {
            $http({
                url: 'api/module/postcomments/getAll',
                method: "POST",
    ...
    $scope.searchDB = function(){
     ...
     $scope.edit_PostComment = function() {
          $scope.error = false;
          var id = $stateParams.id;
          $http({
               url: 'api/module/postcomments/getPostComments/',
               method: "POST",
               params: {id: id}
          }).success(function(data) {
               $scope.postcomments = data.data;
               
                $http.get('api/module/common/getparentpostcomment').success(function(parentpostcommentdata) { //console.log(sourcesdata);
     ...
     $scope.Update_PostComment = function() {
          var id = $stateParams.id;
          $http({
               url: 'api/module/postcomments/editPostComment/',
               method: "POST",
               params: $scope.postcomments
          }).success(function(resdata) {
               if(resdata.success == 1){
                   $scope.success = true; 
                   $(".show-success").text('');
                   $(".show-success").text(resdata.error_code);
                   $scope.activePath = $location.path('/posts/postcomments/' + $scope.postcomments.post_id); 
               } else {
    ...
    /* $scope.Add_PostComment = function() { 
          ...
          $http({
               url: 'api/module/postcomments/addPosts/',
               method: "POST",
     ...
     $scope.Delete_Posts = function(data) {
          ...
          if (deletePlaylists) {
                $http({
                    url: 'api/module/postcomments/deletePostComment',
                    method: "POST",
                    params: {id: data.id}
